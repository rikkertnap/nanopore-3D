...
subroutine VdWcoefficient(lseg,cutoff,Xulimit,sumXu, Xu)
	use system, only : delta
	use const, only : stdout 
	!use molecules
	use MPI, only : ierr, rank
	use transform,  only : MAT
	implicit none

	real*8, intent(in) :: lseg ! medio lseg, radio del segmento
	real*8, intent(in) :: cutoff
	integer, intent(inout) :: Xulimit
	real*8, intent(inout) :: sumXu          
	real*8, intent(inout), allocatable :: Xu(:,:,:)
	...
	do jx = 1, MCsteps
		do jy = 1, MCsteps
			do jz = 1, MCsteps
				...
				if(radio.lt.lseg) cycle ! esta dentro de la esfera del segmento
				...
				matriz(ix, iy, iz) = matriz(ix, iy, iz) + (lseg/radio)**6
...
subroutine kais
	...
	use VdW
	use kai, only : cutoffA, XulimitA, XuA, sumXuA
	use kai, only : cutoffB, XulimitB, XuB, sumXuB
	use chainsdat, only : lsegA,lsegB 


	call VdWcoefficient(lsegA,cutoffA,XulimitA,sumXuA,XuA)
	call VdWcoefficient(lsegB,cutoffB,XulimitB,sumXuB,XuB)
