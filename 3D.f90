subroutine solve(flagcrash)
    ...
    real*8 :: psi(dimx, dimy, dimz)     ! potencial local 
    real*8 :: temp
    ...
    if(infile.eq.0) then
        ...
        do i = ncells+1,(N_poorsolA+N_poorsolB+1)*ncells
            xg1(i)=0.0
            x1(i)=0.0
        enddo
        ...
        if(electroflag.eq.1) then
            ...
            do i=(N_poorsolA+N_poorsolB+1)*ncells+1, (N_poorsolA+N_poorsolB+2)*ncells
                xg1(i)=0.0d0
                x1(i)=0.0d0
            enddo
    ...
    if(rank.ne.0) then
        do
            ...
            if(flagsolver.eq.1) then
                call call_fkfun(x1) ! todavia no hay solucion => fkfun  ==  there is still no solution
            endif ! flagsolver
            if(flagsolver.eq.0) exit ! Detiene el programa para este nodo == stop program for these nodes
        enddo
    endif
    ...
    ! == I recover the value of ier and the norm!
    ! == This way,  know if the solver converged or if they need to change the strategy...

    ! Jefe

    if (rank.eq.0) then
        norma_tosend = norma
        ier_tosend = ier ! distinto tipo de integer  == different type of integer
    ...
    do iz=1,dimz
        do iy=1,dimy
            do ix=1,dimx
                ...
                k = ix+dimx*(iy-1)+dimx*dimy*(iz-1)
               
                xh(ix,iy,iz) = x1(k)

                do ip=1, N_poorsolA
                    xtotalA(ix,iy,iz,ip) = x1( k + ip*ncells )
                enddo

                do ip=1, N_poorsolB
                    xtotalB(ix,iy,iz,ip)=x1( k+ (ip+N_poorsolA)*ncells)
                enddo

                if(electroflag.eq.1) psi(ix,iy,iz) = x1( k+(N_poorsolA+N_poorsolB+1)*ncells)
            enddo
        enddo  
    enddo
