integer function testsystemc(x)
    ...
    use transform
    use channel
    use channelcurved, only : RL=>radiusL, RC => radiusC , LenCSect => lengthsection
   ...
   if (testsystemc.eq.0) then ! saves some time
        ...
        if(curvedflag==0) then ! straight cylinder 
            ...
            if(vect.ge.rchannel**2) then 
                if((v(3).gt.float(RdimZ)*delta).and.(v(3).lt.(float(dimz-RdimZ)*delta))) then  
                    ! == reservoirs in transformed coordinates
                    testsystemc = -1
                    return
                endif
            endif
            ...
            ! ==  reservoirs in transformed coordinates
                ...
                ! zcoor = v(3)-float(dimz)*delta/2.0d0  ! == substract z-origin  channel
                ! zcoor = v(3)-float(Rdimz)*delta        ! == substract reservoir size

                zcoor = mod(v(3)-Rdimz*delta,LenCsect)-LenCsect/2.0d0     
