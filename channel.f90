...
subroutine update_matrix_channel_4(flag)
    ...
    if (verbose.ge.2) then
        ...
        !enddo
        if (rank.eq.0) then
            write(stdout,*) 'channel:', 'update_matrix: Total nanochannel volume real space = ', temp
            write(stdout,*) 'channel:', 'update_matrix: Total discretized volume =',&
...
end subroutine update_matrix_channel_4
...
subroutine update_matrix_channel_3(flag)
    ...
    if (verbose.ge.2) then
        ...
        if (rank.eq.0) then
            ...
            write(stdout,*) 'channel:', 'update_matrix: Total discretized volumen =', & 
                (dimx*dimy*dimz-sum(volprot))*delta**3
            write(stdout,*) 'channel:', 'number of polymers in system =', sumpolseg 
            write(stdout,*) 'channel:', 'surface area =', area
            write(stdout,*) 'channel:', 'surface density =', sumpolseg/area
            write(stdout,*) 'channel:', 'surface density expected from input =', &
                1.0/(((2.0*pi*rchannel)/float(NBRUSH))**2)
        endif
    endif
