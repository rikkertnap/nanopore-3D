...
subroutine readinput
    ...
    character :: basura     ! == tash ued for empty lines
            ...
            case ('systemtype')
                ...
                case(60) ! channel + particle
                    ...
                    do j = 1, NNN
                        ...
                        if(rank.eq.0) write(stdout,*) 'parser:','Set particle',j,&
                            'pos to',  Rellf(1,j), Rellf(2,j), Rellf(3,j)
                    enddo
                    ...
                    do j = 1, NNN
                        ...
                        if(rank.eq.0)write(stdout,*) 'parser:','Set particle',j,&
                            'axis to',  Aell(1,j), Aell(2,j), Aell(3,j)
                    enddo
                    read(fh, *) basura
                ...
                case(1) 
                    ...
                    if(NNN.ne.0) then
                        ...
                        do j = 1, NNN
                            read(fh, *) Rellf(1,j), Rellf(2,j), Rellf(3,j)
                            if(rank.eq.0)write(stdout,*) 'parser:',&
                                'Set particle',j,'pos to',  Rellf(1,j), Rellf(2,j), Rellf(3,j)
                        enddo
                        read(fh, *) basura
                        do j = 1, NNN
                            read(fh, *) Aell(1,j), Aell(2,j), Aell(3,j)
                            if(rank.eq.0)write(stdout,*) 'parser:',&
                                'Set particle',j,'axis to',  Aell(1,j), Aell(2,j), Aell(3,j)
                        enddo
                        read(fh, *) basura
                        do j = 1, NNN
                            read(fh, *) rotmatrix(1,1,j), rotmatrix(1,2,j), rotmatrix(1,3,j)
                        ...
                        do j = 1, NNN
                            read(fh, *) sigma(j)
                            if(rank.eq.0)write(stdout,*) 'parser:',&
                                'Set particle',j,'surface coverage to', sigma(j)
                        enddo
