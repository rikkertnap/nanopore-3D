...
subroutine fcn_flux(x,f,ier2)
    ...
    real*8 :: xpot(dimx, dimy, dimz, N_monomer)
    integer :: id, noffset ! == indices used for flux contrubution to f
    real*8 :: normvol, normel,  normflux 
    ...
    do ix=1,dimx
        do iy=1,dimy
            do iz=1,dimz
                ...
                xh(ix,iy,iz)=x(ix+dimx*(iy-1)+dimx*dimy*(iz-1))  !  == solvent 

                do ip = 1, N_poorsol
                    xtotal(ix,iy,iz,ip) = x(ix+dimx*(iy-1)+dimx*dimy*(iz-1)+ ip*ncells) ! == fraction  polymer of type ip
                enddo
                
                psi(ix,iy,iz)=x(ix+dimx*(iy-1)+dimx*dimy*(iz-1)+(N_poorsol+1)*ncells)   ! == potential 
    ...
    do ix=1,dimx
        do iy=1,dimy
            do iz=1,dimz
                ...
                xpos(ix, iy, iz)  =  x(idx+(N_poorsol+1)*ncells +     ncells)    
                xneg(ix, iy, iz)  =  x(idx+(N_poorsol+1)*ncells + 2 * ncells) 
                xHplus(ix, iy, iz)=  x(idx+(N_poorsol+1)*ncells + 3 * ncells)    
                xOHmin(ix, iy, iz)=  x(idx+(N_poorsol+1)*ncells + 4 * ncells)    
